...
public record AppError(string Message, int StatusCode) {
    ...
    public static AppError Conflict(string message) => new(message, 409);
    public static AppError Unavailable(string message = "Scoring channel unavailable. Retry later.") => new(message, 503);
}
